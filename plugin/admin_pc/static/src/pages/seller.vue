						...
						<mm_card>
							...
							<div class="card_body">
								<mm_form class="bar_filter">
									...
									<mm_list :col="3">
										<mm_item>
											<control_input v-model="query.keyword" title="关键词" desc="商户名称"
											 @blur="search()" />
										</mm_item>
										<mm_item>
											<control_select v-model="query.institution_state" title="认证状态" :options="$to_kv(arr_institution_state)" @change="search()" />
										</mm_item>
										<mm_item>
											<control_select v-model="query.user_id" title="商户持有人" :options="$to_kv(list_account, 'user_id', 'nickname')"
											 @change="search()" />
										</mm_item>
										<mm_item>
								...
								<mm_table type="2">
									<thead class="table-sm">
										<tr>
											...
											<th>
												<control_reverse title="认证状态" v-model="query.orderby" field="institution_state" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="商户持有人" v-model="query.orderby" field="user_id" :func="search"></control_reverse>
											</th>
											<th>
											...
											<th>
												<control_reverse title="商户名称" v-model="query.orderby" field="name" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="注册企业名" v-model="query.orderby" field="institution" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="组织机构码" v-model="query.orderby" field="institution_code" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="详细地址" v-model="query.orderby" field="address" :func="search"></control_reverse>
											</th>
											<th>
												<control_reverse title="经营范围" v-model="query.orderby" field="business" :func="search"></control_reverse>
											</th>
	...
	export default {
		...
		data() {
			return {
				...
				url_import: "/apis/pay/seller?method=import&",
				url_export: "/apis/pay/seller?method=export&",
				field: "seller_id",
				query_set: {
					"seller_id": ""
				},
